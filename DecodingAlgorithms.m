...
%                 HkAll{c} = zeros(N,1);
        ...
        for n=1:N
            ...
            [x_p(:,n+1), W_p(:,:,n+1)] = DecodingAlgorithms.PPDecode_predict(x_u(:,n), W_u(:,:,n), A(:,:,min(size(A,3),n)), Q(:,:,min(size(Q,3),n)));
            x_K=zeros(ns, lags);
    ...
    %            
        ...
        function [x,P]=ukf(fstate,x,P,hmeas,z,Q,R)
            ...
            for k=1:N
              ...
              [x, P] = ukf_example(f,x,P,h,z,Q,R);            % ekf 
            ...
            c=sqrt(c);
            X=DecodingAlgorithms.ukf_sigmas(x,P,c);                            %sigma points around x
            [x1,X1,P1,X2]=DecodingAlgorithms.ukf_ut(fstate,X,Wm,Wc,L,Q);          %unscented transformation of process
            % X1=sigmas(x1,P1,c);                         %sigma points around x1
            % X2=X1-x1(:,ones(1,size(X1,2)));             %deviation of X1
            [z1,Z1,P2,Z2]=DecodingAlgorithms.ukf_ut(hmeas,X1,Wm,Wc,m,R);       %unscented transformation of measurments
        ...
        function [y,Y,P,Y1]=ukf_ut(f,X,Wm,Wc,n,R)
